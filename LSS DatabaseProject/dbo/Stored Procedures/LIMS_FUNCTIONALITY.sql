...
CREATE PROCEDURE [dbo].[LIMS_FUNCTIONALITY]
    @StartDate DATE,
    @EndDate DATE
AS
BEGIN
...
HubFacilityData AS (
    ...
    FROM 
        StandardizedDate c
    WHERE  
        CAST(StandardizedDate AS DATE) BETWEEN @StartDate AND @EndDate
...
Denominator AS (
    SELECT 
        ...
        dist AS district,
         HUB_facility,
        COUNT(HUB_facility) AS #denominator
    FROM 
        HubFacilityData
    GROUP BY 
        HUB_facility, dist,prov
),
...
NumeratorPart AS (
    SELECT 
		prov as province,
        dist AS district,
        HUB_Creator_captured,
        COUNT(HUB_Creator_captured) AS #num
    FROM 
        HubFacilityData
    GROUP BY 
        HUB_Creator_captured, dist,prov
),
...
StandAlones AS (
    SELECT 
        ...
        dist AS district,
        HUB_facility,
        COUNT(HUB_facility) AS #standcount
    FROM 
        HubFacilityData
    WHERE 
        Status = 'Standalone Hubs'
    GROUP BY 
        HUB_facility, dist,prov
),
...
AllDataPoints AS (
    SELECT 
	    d.province,
        d.district AS DenoDist,
        n.district AS NumDist,
        s.district AS StandDist,
        d.HUB_facility,
        ISNULL(d.[#denominator], 0) AS #denominator,
        n.HUB_Creator_captured,
        ISNULL(n.#num, 0) AS #num,
        s.HUB_facility AS stand_Hub_Facility,
        ISNULL(s.#standcount, 0) AS #standcount
    FROM 
        Denominator d
    LEFT JOIN 
        NumeratorPart n ON d.HUB_facility = n.HUB_Creator_captured
    LEFT JOIN 
        StandAlones s ON d.HUB_facility = s.HUB_facility
)
...
SELECT 
    ...
    #num,
    #standcount,
    (CAST(#num AS INT) + CAST(#standcount AS INT)) AS Numerator,
    ROUND(
        CASE 
            WHEN #denominator = 0 THEN 0
            ELSE CAST((CAST(#num AS INT) + CAST(#standcount AS INT)) AS FLOAT) / #denominator
        END, 2
    ) AS Utilization
FROM 
    AllDataPoints
